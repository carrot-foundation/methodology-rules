...
export const ERROR_MESSAGES = {
  ...
  CREDIT_ORDER_DOCUMENT_NOT_FOUND: `The "${DocumentType['Credit Order']}" document was not found.`,
  FAILED_BY_ERROR: 'Unable to calculate rewards distribution.',
  INVALID_CREDIT_UNIT_PRICE: `The "${DocumentEventAttributeName['Credit Unit Price']}" attribute in the "${DocumentType['Credit Order']}" document is invalid.`,
  REWARDS_DISTRIBUTION_RULE_RESULT_CONTENT_NOT_FOUND: (documentId: string) =>
    `The "${documentId}" document has no "${DocumentEventAttributeName['Rule Result Details']}" attribute.`,
} as const;
