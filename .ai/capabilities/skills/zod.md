...
when_to_use:
  - When creating or modifying Zod schemas
  - When working with runtime validation
  - When defining types that need both compile-time and runtime checks
workflow:
  - Define the Zod schema with specific field constraints
  - Derive the TypeScript type using z.infer
  - Add parse/safeParse calls at validation boundaries
  - Create test stubs using zocker if needed
inputs:
  - Data structure requirements or existing type to convert
  - Validation context (external input vs internal data)
outputs:
  - Zod schema definition with derived TypeScript type
references:
  - package.json
---

# Zod Schema Management Skill
