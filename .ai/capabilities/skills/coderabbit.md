...
when_to_use:
  - When CodeRabbit review comments need to be addressed
  - When integrating automated review feedback on a PR
  - When asked to fix CR comments
workflow:
  - Read all CodeRabbit comments from the PR
  - Assess each comment for validity
  - Implement fixes for valid issues
  - Dismiss false positives with explanations
  - Run quality checks and push fixes
inputs:
  - PR number with CodeRabbit comments
  - Optional specific comments to focus on
outputs:
  - Fixed code addressing valid comments
  - Responses to dismissed comments
references:
  - package.json
---

# Address CodeRabbit Review Skill
