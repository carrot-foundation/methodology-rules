...
when_to_use:
  ...
  - When asked to check a branch before merging
workflow:
  - Read the full diff between the branch and main
  - Check for correctness, test coverage, naming, and convention violations
  - Run lint, type-check, and test quality gates
  - Provide structured feedback grouped by severity
inputs:
  - PR number or branch name to review
  - Optional focus areas or specific concerns
outputs:
  - Structured review with findings grouped by severity (critical, suggestion, nit)
references:
  - package.json
---
